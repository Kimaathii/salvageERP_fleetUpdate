    ...
    if (DB_num_rows($Result) == 0) {
        ...
        echo '<p class="page_title_text">
            ...
            <thead>
                <tr>
                    ...
                    <th>' . _('Debit Amount in') . ' ' . $BankDetailRow['currcode'] . '</th>
                    <th>' . _('Credit Amount in') . ' ' . $BankDetailRow['currcode'] . '</th>
                    <th>' . _('Balance') . ' ' . $BankDetailRow['currcode'] . '</th>';
        if ($BankDetailRow['currcode'] != $_SESSION['CompanyRecord']['currencydefault']) {
            echo '<th>' . _('Debit Amount in') . ' ' . $_SESSION['CompanyRecord']['currencydefault'] . '</th>
            <th>' . _('Credit Amount in') . ' ' . $_SESSION['CompanyRecord']['currencydefault'] . '</th>
        ...
        while ($MyRow = DB_fetch_array($Result)) {
            ...
            if ($_POST['ShowType'] == 'All' or ($_POST['ShowType'] == 'Unmatched' and $Matched == _('No')) or ($_POST['ShowType'] == 'Matched' and $Matched == _('Yes'))) {
                ...
                // Set Debit and Credit amounts:
                if ($MyRow['amount'] > 0) {
                    $Debit = locale_number_format($MyRow['amount'], $BankDetailRow['decimalplaces']);
                    $Credit = ''; // No credit for positive amounts
                    $DebitTotal += $MyRow['amount'];
                } else {
                    $Debit = '';
                    $Credit = locale_number_format($MyRow['amount'], $BankDetailRow['decimalplaces']); // Show negative sign in Credit
                    $CreditTotal += abs($MyRow['amount']);
                }
                
                // For local currency amounts:
                if ($BankDetailRow['currcode'] != $_SESSION['CompanyRecord']['currencydefault']) {
                    $DebitLocal = locale_number_format($MyRow['amount'] / $MyRow['functionalexrate'] / $MyRow['exrate'], $_SESSION['CompanyRecord']['decimalplaces']);
                    $CreditLocal = locale_number_format($MyRow['amount'] / $MyRow['functionalexrate'] / $MyRow['exrate'], $_SESSION['CompanyRecord']['decimalplaces']);
                    $DebitTotalLocal += abs($MyRow['amount'] / $MyRow['functionalexrate'] / $MyRow['exrate']);
                    $CreditTotalLocal += abs($MyRow['amount'] / $MyRow['functionalexrate'] / $MyRow['exrate']);
                } else {
                    $DebitLocal = '';
                    $CreditLocal = '';
                }
                ...
                if ($BankDetailRow['currcode'] != $_SESSION['CompanyRecord']['currencydefault']) {
                    echo '<td class="number">' . $DebitLocal . '</td>
                          <td class="number">' . $CreditLocal . '</td>
        ...
        echo '<tr class="striped_row">
                <td colspan="7" style="text-align:right"><strong>' . _('Total') . ':</strong></td>
                <td class="number"><strong>' . locale_number_format($DebitTotal, $BankDetailRow['decimalplaces']) . '</strong></td>
                <td class="number"><strong>' . locale_number_format($CreditTotal, $BankDetailRow['decimalplaces']) . '</strong></td>
                <td class="number"></td>';
                
        if ($BankDetailRow['currcode'] != $_SESSION['CompanyRecord']['currencydefault']) {
            echo '<td class="number"><strong>' . locale_number_format($DebitTotalLocal, $_SESSION['CompanyRecord']['decimalplaces']) . '</strong></td>
                  <td class="number"><strong>' . locale_number_format($CreditTotalLocal, $_SESSION['CompanyRecord']['decimalplaces']) . '</strong></td>
                  <td class="number"></td>';
        }
