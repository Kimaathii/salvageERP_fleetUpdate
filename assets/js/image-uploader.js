...
const ImageUploadComponent = (() => {
    function createElem(config = {}) {
        ...
        $elem.innerHTML = `
            <div class="image-area" style="position: relative; height: 200px; overflow: hidden;">
                <button class="close-btn" type="button" style="position: absolute; right: 5px; top: 5px; z-index: 2; border: 0; background-color: #e60000; color: white; font-weight: bolder; border-radius: 3px; display: flex; justify-content: center; align-items: center; font-size: 15px; font-family: monospace; height: 24px; width: 24px;">&times;</button>
                ...
                <input type="file" style="display: none;" accept="${config.types == '*' ? 'image/*' : [config.types].flat().join(', ')}"/>
     ...
     * @property {Function|null} onImageRemove	A callback to be executed when an the image is cleared/removed
     ...
     * @param {HTMLDivElement} $imgComponent The input element that contains the file to be previewed
...
customElements.define('image-upload', class extends HTMLElement {
    connectedCallback() {
        const imgComp = ImageUploadComponent.create({maxSize: this.getAttribute('max-size') ?? '300'})
        let inputName = this.getAttribute('name')
        if (inputName) {
            imgComp._fileInput.name = inputName
        }
        this.append(imgComp)
        imgComp._imgSrc = this.getAttribute('src')
    }
})
