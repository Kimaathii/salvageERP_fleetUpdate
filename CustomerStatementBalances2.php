...
if (!isset($_GET['CustomerID']) and !isset($_SESSION['CustomerID'])) {
	prnMsg(_('To display the ledger a customer must first be selected from the customer selection screen'), 'info');
	echo '<br /><div class="centre"><a href="', $RootPath, '/SelectCustomer.php">', _('Select a Customer to show ledger for'), '</a></div>';
...
<table class="selection" width=100%>
	<tr>
		...
		<td><select name="Customer">
			<option selected="selected" value="">' . _('Select customer name') . '</option>';
...
echo 			'</select>
	...
	<tr>
		<td>' . _('Date From') . ':</td>
		<td><input type="date" class="form-control" name="FromDate" maxlength="10" size="11" value="' . Date($_SESSION['DefaultDateFormat'], Mktime(0, 0, 0, Date('m') - $_SESSION['NumberOfMonthMustBeShown'], Date('d'), Date('Y'))) . '" /></td>
		<td>' . _('Date To') . ':</td>
		<td><input type="date" class="form-control" name="ToDate" maxlength="10" size="11" value="' . Date($_SESSION['DefaultDateFormat']) . '" /></td>
	</tr>
...
<br />
<div class="centre">
	<input tabindex="4" type="submit" name="RunReport" value="' . _('Show Customer Ledger') . '" />
</div>
</div>
</form>
';
